...
export function DefaultPlanConfigurationsPanel(): JSX.Element {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Vista de combinaciones posibles */}
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Listado de todas las combinaciones posibles para crear configuraciones. Haz clic en "Crear" para añadir una nueva configuración.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <CombinationsGrid
            ...
            onCreate={(combo) => {
              const formData: UpsertDefaultPlanConfigurationPayload = {
                name: combo.name,
                description: combo.description,
                priority: combo.priority,
                is_active: true,
                main_goal: combo.main_goal,
                training_location: combo.training_location,
                activity_level: combo.activity_level,
                gender: combo.gender,
                min_training_days_per_week: combo.min_training_days_per_week,
                max_training_days_per_week: combo.max_training_days_per_week,
              ...
              handleOpenDialog("create", undefined, formData)
            }}
            onEdit={(config) => {
              handleOpenDialog("edit", config)
            }}
          />
        </CardContent>
      </Card>
...
interface CombinationsGridProps {
  existingConfigurations: DefaultPlanConfiguration[]
  onCreate: (combo: Combination) => void
  onEdit: (config: DefaultPlanConfiguration) => void
}

function CombinationsGrid({ existingConfigurations, onCreate, onEdit }: CombinationsGridProps) {
  const mainGoals = ['lose_weight', 'gain_muscle', 'body_recomposition', 'maintenance']
  const trainingLocations = ['home', 'gym', 'outdoor']
  const activityLevels = ['sedentary', 'light', 'moderate', 'active', 'very_active']
  const trainingDaysOptions = [
    { min: 1, max: 3, label: '1-3 días' },
    { min: 3, max: 5, label: '3-5 días' },
    { min: 5, max: 7, label: '5-7 días' },
  ]
  ...
  const combinations: Combination[] = useMemo(() => {
    ...
    // Combinaciones básicas: Objetivo + Lugar + Actividad
    for (const goal of mainGoals) {
      for (const location of trainingLocations) {
        for (const activity of activityLevels) {
          const name = `${goal === 'lose_weight' ? 'Pérdida peso' : 
                        goal === 'gain_muscle' ? 'Ganancia músculo' : 
                        goal === 'body_recomposition' ? 'Recomposición' : 'Mantenimiento'} - ${location === 'home' ? 'Casa' : location === 'gym' ? 'Gimnasio' : 'Aire libre'} - ${activity === 'sedentary' ? 'Sedentario' : activity === 'light' ? 'Ligero' : activity === 'moderate' ? 'Moderado' : activity === 'active' ? 'Activo' : 'Muy activo'}`
          
          combos.push({
            name,
            description: `Configuración para ${name.toLowerCase()}`,
            priority: priority++,
            main_goal: goal,
            training_location: location,
            activity_level: activity,
            gender: null,
            min_training_days_per_week: activity === 'sedentary' ? 1 : activity === 'light' ? 2 : activity === 'moderate' ? 3 : activity === 'active' ? 4 : 5,
            max_training_days_per_week: activity === 'sedentary' ? 3 : activity === 'light' ? 4 : activity === 'moderate' ? 5 : activity === 'active' ? 6 : 7,
            age_min: null,
            age_max: null,
          })
        }
  ...
  const findExisting = (combo: Combination) => {
    return existingConfigurations.find(config => 
      config.main_goal === combo.main_goal &&
      config.training_location === combo.training_location &&
      config.activity_level === combo.activity_level
    )
  }
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        Se muestran {combinations.length} combinaciones posibles. Las que ya existen están marcadas.
      </p>
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3 max-h-[600px] overflow-y-auto">
        {combinations.map((combo, idx) => {
          const existing = findExisting(combo)
          const exists = !!existing
          return (
            <Card key={idx} className={exists ? "border-primary/50 bg-primary/5" : "hover:border-primary"}>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm">{combo.name}</CardTitle>
                <CardDescription className="text-xs">
                  Prioridad: {existing?.priority ?? combo.priority}
                  {existing && (
              ...
              <CardContent className="space-y-2">
                <div className="flex flex-wrap gap-1">
                  {combo.main_goal && <Badge variant="secondary" className="text-xs">{combo.main_goal}</Badge>}
                  {combo.training_location && <Badge variant="outline" className="text-xs">{combo.training_location}</Badge>}
                  {combo.activity_level && <Badge variant="outline" className="text-xs">{combo.activity_level}</Badge>}
                </div>
                <div className="text-xs text-muted-foreground">
                  Días: {combo.min_training_days_per_week}-{combo.max_training_days_per_week}
                </div>
                {existing && (
                  <div className="pt-2 space-y-1 text-xs">
                    {existing.default_nutrition_plan && (
                      <div className="text-muted-foreground">
                        Nutrición: <span className="font-medium">{existing.default_nutrition_plan.name}</span>
                      </div>
                    )}
                    {existing.default_workout_program && (
                      <div className="text-muted-foreground">
                        Entrenamiento: <span className="font-medium">{existing.default_workout_program.name}</span>
                      </div>
                    )}
                  </div>
                )}
