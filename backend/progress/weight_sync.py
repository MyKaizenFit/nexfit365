...
def upsert_weight_entry(
    ...
    *,
    ...
    """
    Crea o actualiza la entrada de peso más reciente para un usuario y fecha.
    """
    ...
    entry = (
        WeightEntry.objects.filter(user=user, date=date)
        .order_by("-created_at")
        .first()
    )
    ...
    if update_user_weight:
        try:
            ...
            logger.warning("No se pudo actualizar user.weight para user=%s: %s", user.pk, exc)
