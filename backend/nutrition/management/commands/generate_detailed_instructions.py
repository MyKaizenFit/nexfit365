"""
Comando de gestión para generar instrucciones detalladas para recetas con instrucciones genéricas
"""
...
class Command(BaseCommand):
    help = 'Genera instrucciones detalladas para recetas con instrucciones genéricas'

    def handle(self, *args, **options):
        self.stdout.write("=" * 70)
        self.stdout.write(self.style.SUCCESS("🍽️  GENERANDO INSTRUCCIONES DETALLADAS PARA RECETAS"))
        self.stdout.write("=" * 70 + "\n")
        ...
        for recipe in all_recipes:
            if recipe.instructions:
                instructions_lower = recipe.instructions.lower().strip()
                if (instructions_lower == 'seguir preparación indicada.' or 
                    instructions_lower == 'seguir preparación indicada' or
                    (len(recipe.instructions.strip()) == 28 and 'seguir preparación' in instructions_lower)):
                    generic_recipes.append(recipe)
        
        self.stdout.write(f"📋 Encontradas {len(generic_recipes)} recetas con instrucciones genéricas\n")
        ...
        for recipe in generic_recipes:
            try:
                ...
                self.stdout.write(self.style.SUCCESS(f"✅ {recipe.name}"))
                self.stdout.write(f"   Instrucciones generadas: {len(new_instructions.split(chr(10)))} pasos")
                updated += 1
                
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"❌ Error en {recipe.name}: {str(e)}"))
                errors += 1
        
        self.stdout.write("\n" + "=" * 70)
        self.stdout.write(self.style.SUCCESS(f"✅ Actualizadas: {updated} recetas"))
        if errors > 0:
