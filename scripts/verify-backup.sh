...
SUMMARY=$(docker exec "$DB_CONTAINER" psql -U "$DB_USER" -d "$TMP_DB" -At -F '|' -c "SELECT 'workouts_exercise', COUNT(*) FROM workouts_exercise UNION ALL SELECT 'workouts_workoutprogram', COUNT(*) FROM workouts_workoutprogram UNION ALL SELECT 'nutrition_food', COUNT(*) FROM nutrition_food UNION ALL SELECT 'nutrition_recipe', COUNT(*) FROM nutrition_recipe UNION ALL SELECT 'nutrition_nutritionplan', COUNT(*) FROM nutrition_nutritionplan;")
